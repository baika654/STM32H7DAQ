...
void TIM1Init() {
	...
	LL_GPIO_SetAFPin_0_7(GPIOC, LL_GPIO_PIN_0, LL_GPIO_AF_2);
	//PA9 to TIM1 CH2
	...
	LL_GPIO_SetAFPin_0_7(GPIOC, LL_GPIO_PIN_3, LL_GPIO_AF_2);

	//Configure the Timer
	LL_TIM_SetCounterMode(TIM1, LL_TIM_COUNTERMODE_UP);
	LL_TIM_SetPrescaler(TIM1, 149);		//Reduce timer from full clock speed, SystemCoreClock, to SystemCoreClock / 150 = 1 MHz
	uint32_t arr = __LL_TIM_CALC_ARR(SystemCoreClock, LL_TIM_GetPrescaler(TIM1), 100);		//Set output clock to 100 Hz
	LL_TIM_SetAutoReload(TIM1,  arr);
	LL_TIM_EnableARRPreload(TIM1);
	LL_TIM_SetClockDivision(TIM1, LL_TIM_CLOCKDIVISION_DIV1);

	//Configure TIM output
	LL_TIM_OC_SetMode(TIM1, LL_TIM_CHANNEL_CH1, LL_TIM_OCMODE_PWM1);
	LL_TIM_OC_SetPolarity(TIM1, LL_TIM_CHANNEL_CH1, LL_TIM_OCPOLARITY_HIGH);
	LL_TIM_OC_SetCompareCH1(TIM1, 5000);
...
//	LL_TIM_CC_EnableChannel(TIM1, LL_TIM_CHANNEL_CH2);
	...
	LL_TIM_OC_SetCompareCH3(TIM1, 5000);
...
//	LL_TIM_CC_EnableChannel(TIM1, LL_TIM_CHANNEL_CH3);
	...
	LL_TIM_OC_SetCompareCH4(TIM1, 5000);
...
/**
  * @brief	This function sets the output frequency of TIM1, sets the ARR and CCR values
  * @param	freq: Desired timer frequency on PWM output
  * @return	None
  */
void TIM1SetFreq(uint32_t freq) {
	uint32_t arr = __LL_TIM_CALC_ARR(SystemCoreClock, LL_TIM_GetPrescaler(TIM1), freq);		//Set output clock to 100 Hz
	LL_TIM_SetAutoReload(TIM1,  arr);
}

/**
  * @brief	This function sets the output frequency of TIM2, sets the ARR value
  * @param	freq: Desired timer frequency of TIM update calls
  * @return	None
  */
void TIM2SetFreq(uint32_t freq) {
	LL_TIM_SetAutoReload(TIM2, __LL_TIM_CALC_ARR(SystemCoreClock, LL_TIM_GetPrescaler(TIM2), freq));	//Set output clock to frequency
}
...
/**
  * @brief	This function sets the TIM1 ARR value
  * @param	arr: Desired timer ARR value
  * @return	None
  */
...
/**
  * @brief	This function sets the TIM1 output channel compare value
  * @param	channel: Output channel to set
  * @param	value: Compare value in Q1.15
  * @return	None
  */
void TIM1SetCompare(uint8_t channel, uint16_t value) {
	uint16_t ccr = (LL_TIM_GetAutoReload(TIM1) * value) >> 15;
	switch(channel) {
		case 1:
...
/**
  * @brief	This function enables/disables the TIM1 output channel
  * @param	channel: Output channel to set
