...
static int8_t CDC_Init_HS(void)
{
  ...
  //USBD_CDC_SetRxBuffer(&hUsbDeviceHS, UserRxBufferHS);
  	...
  	USBD_CDC_SetTxBuffer(&hUsbDeviceHS, usbTXBuffer, 0);
...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  if(*Len == 64) {
  		...
  		USBD_CDC_SetRxBuffer(&hUsbDeviceHS, &usbRXBuffer[usbRXBufferLength]);
  	}
  	else {
  		...
  		USBD_CDC_SetRxBuffer(&hUsbDeviceHS, &usbRXBuffer[0]);
  	}
